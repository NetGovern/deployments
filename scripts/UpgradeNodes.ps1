...
ForEach ($node_type in ('index','archive','dp')) {
    $info_hash[$node_type].psobject.Properties | foreach-object {
        $node_ip = $_.Name
        $_.Value.psobject.properties | ForEach-Object {
            if ( $($_.Name) -eq "version" -and $($_.Value) -lt $upgrade_version ) {
                if ($node_type -eq "dp") {
                    $upgrade_me = $True
                    $current_version_dp = $($_.Value)
                }
                if ($node_type -eq "archive") {
                    ...
                    Write-Output "Starting Archive upgrade job"
                    Invoke-Command -ScriptBlock $install_msi -ComputerName $node_ip -Credential $admin_credentials -AsJob -JobName $job_name
                    if ($?) { $jobs += $job_name }
                    else { Write-output "Job could not be launched at $node_ip, please run upgrade manually" }
                }
                if ($node_type -eq "index") {
                    ...
                    $scriptBlock = [Scriptblock]::Create($install_rpms_command)
                    $job_name = "JobAt$($node_ip)-$(Get-Date -f "MMddhhmmss")"
                    Write-Output "Starting Index upgrade job"
                    Start-Job -Name $job_name -ScriptBlock $scriptBlock
                    if ($?) { $jobs += $job_name }
                    else { Write-output "Job could not be launched at $node_ip, please run upgrade manually" }
                }
            }
...
if ($upgrade_me) {
    ...
    Write-Output "Backing up existing DP folders to $($env:NETMAIL_BASE_DIR)\..\bkp_dp_$current_version_dp"
    (New-Item -Path "$($env:NETMAIL_BASE_DIR)\.." -Name "bkp_dp_$current_version_dp" -ItemType "directory") | Out-Null
