...
#Configure Nipe for multitenancy
$nipeSearcher = Get-content -Path ("$env:NETMAIL_BASE_DIR\..\Nipe\Config\nipeSearcher.properties")
$nipeSearcher = $nipeSearcher.Replace("ignoreTenantId=true","ignoreTenantId=false")
$nipeSearcher | Out-File -FilePath "$env:NETMAIL_BASE_DIR\..\Nipe\Config\nipeSearcher.properties" -Encoding ascii
...
Write-Output "Waiting for APId https://localhost:444/docs"
$curlExe = "$env:NETMAIL_BASE_DIR\etc\scripts\setup\curl.exe"
$curlParamsHttpCode = '-k', '-s', '-w', '"%{http_code}"', '-o', '/dev/null', "https://localhost:444/docs"
$status = 0
$retries = 5
$retry = 0
...
while ($status -eq 0 -and $retry -lt $retries) {
    try {
        ...
        Write-Host "Cannot connect to https://localhost:444/docs"
        $status = 0
    }
    if ($status -ne 200) {
        $retry += 1
...
if ($retry -ge $retries) {
    Write-Output "Cannot get to https://localhost:444/docs"
    Write-Output "Last status code: $status"
    Write-Output "Please verify services manually"
}
...
if ($status -eq 200) {
    ...
    while (!$apidKey -and $retry -lt $retries) {
        $apidKey = ..\common\generateApiKey.ps1 -tenantId $tenant_id
        if (!$apidKey) {
            $retry += 1
            Start-Sleep 30
        }
...
if ($retry -ge $retries) {
    Write-Output "Cannot generate api key"
    Write-Output "Please generate it manually"
}

#Setup NetGovern SNMP Agent
Set-Service -Name NetmailSNMPService -StartupType Automatic
Start-Service -Name NetmailSNMPService

Write-Log "`r`nConfigure xgwxmlv.cfg" $logfile -toStdOut
$edirPropertiesNipe = "$env:NETMAIL_BASE_DIR\..\Nipe\Config\edir.properties"
Copy-Item "$env:NETMAIL_BASE_DIR\..\ConfigTemplates\DP\xgwxmlv.cfg" -Destination "$env:NETMAIL_BASE_DIR\..\RemoteProvider" -Force
# NM-24668 -- PDF Export related settings
$xgwxmlvCfg = Get-content -Path ("$env:NETMAIL_BASE_DIR\..\RemoteProvider\xgwxmlv.cfg")
$xgwxmlvCfg = $xgwxmlvCfg -replace "edir.host=.+", `
    "edir.host=$(((Select-String 'edir.host=' $edirPropertiesNipe).Line -split '=',2)[1])"
$xgwxmlvCfg = $xgwxmlvCfg -replace "edir.logindn=.+", `
    "edir.logindn=$(((Select-String 'edir.logindn=' $edirPropertiesNipe).Line -split '=',2)[1])"
$xgwxmlvCfg = $xgwxmlvCfg -replace "edir.port=.+", `
    "edir.port=$(((Select-String 'edir.port=' $edirPropertiesNipe).Line -split '=',2)[1])"
$xgwxmlvCfg = $xgwxmlvCfg -replace "edir.container=.+", `
    "edir.container=$(((Select-String 'edir.container=' $edirPropertiesNipe).Line -split '=',2)[1])"
$xgwxmlvCfg = $xgwxmlvCfg -replace "edir.ssl=.+", `
    "edir.ssl=$(((Select-String 'edir.ssl=' $edirPropertiesNipe).Line -split '=',2)[1])"
$eclientsPassword = "$(((Select-String 'edir.loginpwdclear=' $edirPropertiesNipe).Line -split '=',2)[1])"
$eclientsPasswordEncrypted = ( & $env:NETMAIL_BASE_DIR\etc\scripts\setup\Win-edir\InstallUtils.exe mode=enc value="$eclientsPassword").trim()
$xgwxmlvCfg = $xgwxmlvCfg -replace "edir.loginpwd=.+", "edir.loginpwd=$eclientsPasswordEncrypted"

$xgwxmlvCfg | Out-File -FilePath "$env:NETMAIL_BASE_DIR\..\RemoteProvider\xgwxmlv.cfg" -Encoding ascii
...
if ( $remote_provider_ip_address -eq "0.0.0.0" ) {
    ...
    $dpPort = ..\ConfigureDP\ConfigureDP.ps1 -rp_server_address $remote_provider_ip_address -rp_admin_user $remote_provider_admin_user -rp_admin_password $remote_provider_password -tenant_id $tenant_id -apid_key $apidKey
    Write-Log "Netmail search was configured to listen to port: $dpPort" $logfile -toStdOut
}
if ($status -eq 200) {
    Write-Log "Script Finished.  You can login to https://$ipaddress" $logfile -toStdOut
} else {
    Write-Log "Script Finished.  Please verify services manually before attempting to login to https://$ipaddress" $logfile -toStdOut
}
